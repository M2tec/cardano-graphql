...
CREATE OR REPLACE VIEW testing.ma_tx_first_mint AS
WITH ranked_data AS (
  SELECT *,
...
CREATE OR REPLACE VIEW  testing.assets_with_first_tx AS
SELECT 
...
JOIN 
    testing.ma_tx_first_mint as mint
ON 
    multi.id = mint.ident
JOIN 
    public.tx 
...
CREATE OR REPLACE VIEW testing.metadata_with_logo AS
SELECT 
	metadata.subject,
	metadata.decimals,
...
CREATE OR REPLACE VIEW testing."Asset" AS
SELECT 
    multi."assetId",
    multi."assetName",
	metadata.decimals,
	...
	metadata.url
FROM 
    testing.assets_with_first_tx as multi
LEFT JOIN 
    testing.metadata_with_logo as metadata
ON
